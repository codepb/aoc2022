...
function unionRange(range1: Range, range2: Range): Range | false {
    if (range1[0] <= range2[1] && range1[1] >= range2[0] || range2[0] <= range1[1] && range2[1] >= range1[0] || range1[1] === range2[0] - 1 || range1[0] === range2[1] + 1) {
...
function updateMap(map: Record<number, Range[]>, { beacon, sensor }: ReturnType<typeof parseInstruction>, max: number) {
    const distance = manhattanDistance(beacon, sensor);

    for (let y = Math.max(sensor[1] - distance, 0); y <= Math.min(sensor[1] + distance, max); y++) {

        const currentDistance = manhattanDistance(sensor, [sensor[0], y])
        if (!map[y]) {
            map[y] = [];
        }
        const startRange = Math.max(sensor[0] - distance + currentDistance, 0);
        const endRange = Math.min(sensor[0] + distance - currentDistance, max);
...
const runner: RunnerFunction<number> = (input) => {
    ...
    for (const line of lines) {
        ...
        updateMap(map, instruction, max);
    }
    const y = parseInt(Object.keys(map).find(y => map[y][0][0] !== 0 || map[y][0][1] !== max));
    const x = map[y][0][1] + 1;
    return x * 4000000 + y;
}
